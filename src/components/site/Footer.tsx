...
export function Footer({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  const linksLabel = locale === "ar" ? "الروابط الرسمية" : "Liens officiels";
  const contactLabel = locale === "ar" ? "التسجيل والاستفسار" : "Inscription & contact";

  return (
    <footer className="border-t border-accent/15 bg-[#03080f] px-5 py-10 sm:px-8">
      <div className="mx-auto grid w-full max-w-7xl gap-8 md:grid-cols-[1.1fr_.9fr_.7fr] md:items-start">
        <div>
        ...
        <div className="text-xs text-chalk-dim">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-accent">{linksLabel}</p>
