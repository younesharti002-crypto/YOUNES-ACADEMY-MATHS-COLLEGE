...
export function Positioning({ dict }: { dict: Dictionary }) {
  ...
  const columns = [
    { data: positioning.physics, symbol: "⚠️" },
    { data: positioning.chemistry, symbol: "📱" },
  ];
          ...
          {columns.map(({ data, symbol }, index) => (
            <article key={data.title} className={`rounded-[1.7rem] border p-6 ${index === 0 ? "border-accent/25 bg-accent/[0.07]" : "border-white/10 bg-white/[0.04]"}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-[0.22em] text-accent/75">{data.label}</p>
